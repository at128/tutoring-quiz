...
| **Claude Code** | **M0** (scaffold, Docker, CI) and **B1** (domain rules, persistence, cookie auth, seeding), and started F1. After its usage limit reset, it did the following. *Reviewed and resumed:* read Codex's B2/B3, committed Codex's pending verified fix, created the GitHub repo and replayed the history as PRs #1–#5. *Built the UI:* **F2** (student UI, PR #6) and **F3** (teacher UI, PR #7), both from the prototype and compared with it in a headless browser at 360 px and on desktop. *Added:* the **live demo** (PR #8) and the **login limit fix** (PR #9). *Drafted docs:* README, DECISIONS, this file and the workflow docs. *On Thursday it also built:* the full **Arabic interface** (PR #12); the **M4 fixes** and Codex’s browser tests in CI (PR #13); live-demo **backups**, the **score floor at 0** and one-command **run scripts** (PR #14). It merged each PR only after CI was green. |
| **OpenAI Codex (VS Code)** | Took over implementation while Claude Code was at its limit. It finished **F1** (sign-out failure handling, the session-check error state) and implemented **B2** (student attempts) and **B3** (teacher authoring and results), with HTTP integration tests on real SQLite files. It later ran a **backend/frontend handler audit** (`docs/reviews/backend-frontend-handler-audit-codex-review.md`) with new edge-case tests and fixes, in parallel with F3. **M4** (the adversarial whole-repo review, `docs/reviews/M4-codex-adversarial.md`): four findings, a cross-site request gap, two editor states and a stale count. It then started browser E2E tests and Arabic-content HTTP tests, but reached its usage limit before committing them; Claude Code committed that work and fixed the findings (PR #13). |
...
- I approved changing the login limit from per IP to **per IP + username**, and at first kept **browser E2E tests out of scope**. Later on Thursday I reversed that and asked for browser E2E tests in CI.
...
- **Automated:**
  ...
  - Frontend: lint, typecheck, unit tests (including the Arabic interface and its dictionaries), build.
  - Browser: Playwright tests in `frontend/e2e` (Arabic phone and English desktop journeys, a teacher-to-student journey, editor edge cases) against the real Docker container.
  - GitHub Actions runs all of these (jobs `backend`, `frontend`, `docker`, `e2e`) on every PR and push.
...
- **Visual and journey checks:**
  ...
  - Those development scripts stayed outside the repo; the journeys that matter most became the Playwright tests above, which CI runs.
- **Independent review:** Codex’s handler audit and its M4 adversarial review. Every accepted finding was fixed with a regression test.
- **My own checks:** I used the live demo during development and set up an extra tester account (Aya) on 24 Sep. A formal real-phone checklist wasn’t written down, so none is claimed here.
...
| Audit (in place of C3) | Codex, fixes by both | handler audit: list refresh at the next opening/closing (Codex's fix, in PR #7); impossible or DST-gap editor times and truncated seconds (fixed by Claude Code, PR #7); stale cookie after a user is deleted and an ambiguous-save state in autosave (Codex’s fixes, PR #11) | triaged the findings |
| M4 | Codex (review), Claude Code (fixes) | adversarial review with four findings; Codex’s browser E2E and Arabic-content tests committed by Claude Code after Codex’s limit; fixes: same-origin guard, editor time check, class-list retry, finalized count (PRs #12, #13) | accepted all four findings and the blank-text rule |
...
- **Claude Code agreed to Codex's proposal to add browser E2E tests without noticing `PLAN.md` lists them as out of scope.** It was caught before any work started, and I kept them out at the time. Later that day I decided the journeys were worth automating, and they were added to CI (PR #13).
...
- Drafted by Claude Code from what caused friction:
