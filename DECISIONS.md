...
| Negative marking model | Per quiz, one of: none; a **percentage** of each question's points per wrong answer (e.g. 25 %); or a **fixed mark** per wrong answer (e.g. 0.5), never more than the question is worth. Unanswered = 0. | Works with per-question points and matches "varies by teacher and by quiz" without per-question settings. The fixed mark was Atta's addition (24 Sep): one amount for the whole quiz, capped at each question's points, and the cap is explained in the editor. |
| Can the total go below zero? | **No: it stops at 0.** Each wrong answer still takes points back from what the student earned, but never more. The result screen says so when it happens. | The first version allowed negative totals. Atta decided on 24 Sep that a score below zero shouldn't exist: it confuses students and parents, and a 0 already says "guessing didn't pay". A data migration brought any stored negative total to 0. |
...
| Editing a quiz after students started | **While it is open** (server time, up to and including the closing instant): content is frozen once any attempt exists; unpublish/delete are blocked. **After it closes** the owner may correct questions, options, the answer key, points and marking; the save regrades every attempt in the same transaction. Its classes, dates and time limit stay as they were. Drafts are fully editable. | Changing a correct answer mid-quiz would make scores unfair; after the close, correcting a wrong answer key is exactly what a teacher needs (Atta, 24 Sep). Classes and dates stay fixed because the attempts were taken under them: removing a class would hide its results, and reopening would let others take a quiz whose results are already out. |
| What a regrade keeps | Every attempt keeps its status (Submitted/Expired), start and finish times, and the options the student chose. Only the score and counts change, and the attempt records when (shown as "recalculated"). Questions and options keep their identity when edited; a removed one is only marked removed, so the answers stay readable. | Nothing a student did is rewritten; the teacher can always see what each student chose. |
| A student chose an option the teacher later removed | That answer counts as **unanswered**: 0, no deduction. The teacher still sees what was chosen. | A student shouldn't lose points for an option that no longer exists. |
| Can a teacher hide scores? | Yes, per quiz ("Show scores to students", on by default, existing quizzes included), at any time. While hidden, student requests return no score, percentage or counts at all; showing them again reveals the stored result. | Some teachers want to review or correct before students see marks (Atta, 24 Sep). Hiding in the page alone would leak the score through the API. |
...
- **Teachers see each student's answers** (from Results → View answers): the chosen and the correct option, correct/wrong/unanswered, points earned and deducted per question, scored by the same function as the stored result, so the two always agree.
...
- **Any teacher can assign a quiz to any class.** There is no teacher-to-class restriction; a centre with fixed teacher-class pairs would add one.
- **Accounts and classes come only from the seed files.** Every seeded student shares one demo password (every teacher another), there is no password change, and the seeder only runs on an empty database, so adding a student later means editing the data directly or starting over. The admin role in §5 fixes both before real use.
