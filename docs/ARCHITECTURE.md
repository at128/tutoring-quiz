...
- CSRF: `SameSite=Strict`, plus a same-origin guard (`Auth/SameOriginGuard.cs`, added after the M4 review). An `/api` request that changes something (any method but GET/HEAD/OPTIONS/TRACE) is refused with 403 `auth.forbidden` when the browser labels it as coming from another page (`Sec-Fetch-Site` not `same-origin`), or, for older browsers, when its `Origin` differs from the site. SameSite alone trusts sibling subdomains, and the bodyless actions (publish, start, submit) accept a plain HTML form. Requests with neither header (curl, tests) carry no victim's cookie and pass.
