...
import { availableTimeText, liveQuiz, liveQuizList, shortTimeWarning } from './liveQuiz'
...
describe('liveQuiz', () => {
  it('does not call a still-open sub-minute window zero usable minutes', () => {
    expect(availableTimeText(0)).toBe('less than 1 minute')
    expect(availableTimeText(1)).toBe('1 minute')
    expect(availableTimeText(8)).toBe('8 minutes')
    expect(shortTimeWarning(8)).toBe('only 8 minutes')
    expect(shortTimeWarning(1)).toBe('only 1 minute')
    expect(shortTimeWarning(0)).toBe('less than 1 minute')
  })
