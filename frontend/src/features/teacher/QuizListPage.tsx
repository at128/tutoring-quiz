...
import { hasResults, markingShort, progressOf, quizCount } from './teacherCopy'
    ...
    <PageShell width="teacher" mainClassName="gap-6 pt-5 pb-10 md:pt-8 md:pb-12">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div className="flex flex-col gap-1">
          <h1 className="text-page leading-[1.3] font-bold md:text-display">My quizzes</h1>
          {quizzes.data && (
            <p className="text-small text-muted">
              {quizCount(quizzes.data.length)}
              <span className="hidden md:inline"> · sorted Open, Scheduled, Draft, Closed</span>
            </p>
          )}
        </div>
        ...
        <>
          ...
          {quizzes.data.some((q) => q.isLocked) && (
            <p className="flex items-start gap-2 text-small leading-normal text-muted">
              ...
              Locked quizzes have student attempts: their content can’t change, and they can’t be unpublished or deleted.
            </p>
          )}
        </>
      )}
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      <span className="text-meta text-muted">
        <Num>{plural(quiz.questionCount, 'question')}</Num> · <Num>{plural(quiz.maxScore, 'pt', 'pts')}</Num>
        {long && (
          <>
            {' '}
            · <Num>{quiz.durationMinutes}</Num> min · {quiz.wrongAnswerPenaltyPercent === 0 ? 'no negative marking' : `−${quiz.wrongAnswerPenaltyPercent}% per wrong`}
          </>
        )}
      </span>
    </div>
...
function QuizTable({ quizzes, nowMs }: { quizzes: TeacherQuizSummary[]; nowMs: number }) {
  return (
    <div role="table" aria-label="My quizzes" className="hidden rounded-sheet border border-rule bg-paper md:block">
      <div role="row" className={`${columns} rounded-t-sheet border-b border-rule bg-desk px-5 py-3`}>
        {['Quiz', 'State', 'Window', 'Time and marking', 'Progress'].map((h) => (
      ...
      {quizzes.map((quiz, index) => {
        const progress = progressOf(quiz, nowMs)
        return (
          <div key={quiz.id} role="row" className={`${columns} items-center px-5 py-4 ${index > 0 ? 'border-t border-rule-soft' : ''}`}>
            ...
            <div role="cell" className="text-small leading-[1.55] text-ink-2">
              <div>Opens {formatDateTime(quiz.opensAt)}</div>
              <div>Closes {formatDateTime(quiz.closesAt)}</div>
            </div>
            <div role="cell" className="text-small leading-[1.55] text-ink-2">
              <div>
                <Num>{quiz.durationMinutes}</Num> min
              </div>
              <div>{markingShort(quiz.wrongAnswerPenaltyPercent)}</div>
            </div>
            <div role="cell" className="flex flex-col gap-0.5">
              <span className="text-body font-bold">{progress.value ? <Num>{progress.value}</Num> : '—'}</span>
  ...
  return (
    <article className="flex flex-col gap-3 rounded-sheet border border-rule bg-paper p-4">
      ...
      <dl className="flex flex-col">
        <Row label="Opens">{formatDateTime(quiz.opensAt)}</Row>
        <Row label="Closes">{formatDateTime(quiz.closesAt)}</Row>
        <Row label="Progress">
          {progress.value ? (
            <>
              <strong>
                <Num>{progress.value}</Num>
              </strong>{' '}
              {progress.caption}
            </>
          ) : (
...
function NoQuizzes() {
  return (
    <div className="flex flex-col items-center gap-3 rounded-sheet border border-dashed border-strong bg-paper px-5 py-8 text-center">
      ...
      <h2 className="text-card font-bold">No quizzes yet</h2>
      <p className="max-w-[300px] text-[15px] leading-[1.55] text-ink-2">
        Create your first quiz. It stays a draft, hidden from students, until you publish it.
      </p>
...
function ListSkeleton() {
  return (
    <div className="flex flex-col gap-3 rounded-sheet border border-rule bg-paper p-4" aria-busy="true" aria-label="Loading your quizzes">
      {[0, 1, 2].map((i) => (
